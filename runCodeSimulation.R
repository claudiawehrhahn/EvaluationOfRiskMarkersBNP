...
# Run Code: Simulated data sets
# This code has to be executed for E=1 and E=2, which denote each simulated Scenario
...
grid01 <- seq(0.01, 0.99, by = 0.01) 
grid01 <- cbind(grid01, grid01)
ngrid01 <- nrow(grid01)

# grid for covariate:
xgrid <- seq(0.01, 0.99, by = 0.01)
ngrid <- length(xgrid)
...
# grid of covariate:
xgrid <- seq(0.01, 0.99, by = 0.01) # seq(0.01, 0.99, by = 0.01)
ngrid <- length(xgrid)
...
print(mean(abs(kendallsTauE1Estimate - trueKendallsTauE1)))
